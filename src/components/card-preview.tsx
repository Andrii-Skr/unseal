...
function CardPreviewContent({ card, className }: CardPreviewProps) {
  ...
  const stageLabels: Record<(typeof PREVIEW_STAGES)[number], string> = {
    intro: t("intro"),
    "lock-1-opened": t("lock1"),
    "lock-2-opened": t("lock2"),
    "lock-3-opened": t("lock3"),
    "lock-4-opened": t("lock4"),
    "all-locks-opened": t("unlocked"),
    "entering-heart": t("inside"),
    "inside-heart": t("inside"),
    "final-message": t("final"),
  };

  return (
    <section className={cn("flex min-h-0 flex-col", className)}>
      <div
        aria-label={t("stageGroup")}
      ...
      <div className="paper-surface relative min-h-0 flex-1 overflow-hidden rounded-[1.8rem] border border-[var(--rose-deep)]/15 shadow-[0_18px_55px_rgba(109,67,80,.14)]">
        {inside ? (
          <InsideHeartScene compact locale={card.language}>
            {stage === "final-message" ? (
